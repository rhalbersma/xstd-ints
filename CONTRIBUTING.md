...
Those dependencies are the test suite's rather than the package's, and [`vcpkg.json`](vcpkg.json) still declares `test` a *default* feature, so manifest mode resolves Boost with no feature named. That is deliberate and it is load-bearing: the shared CI workflows configure through the vcpkg toolchain without setting `VCPKG_MANIFEST_FEATURES`, so an opt-in feature would leave every leg without Boost.Test. Read the manifest as this repository's development manifest, not as packaging metadata for a consumer; the library itself has no dependency to declare. Nothing in it pins a `builtin-baseline` either, so the version each dependency resolves to is whatever the vcpkg checkout at `VCPKG_ROOT` carries; pin that checkout if you need one build to reproduce another.
